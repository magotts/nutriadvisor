...
const Exercise_Search = () => {
  ...
  const getExercise = () => {
    if (query !== "" && gender!== "" && weight_kg!== "" && height_cm !== "" && age !== "") {
      const headers = {
        'x-app-id': '5ea2186d',
  ...
  return (
    ...
    <div className="form_center">
      ...
      <form onSubmit={onSubmit}>
        ...
        <div>
          <input type="text" value={weight_kg} placeholder="Weight (kg)" onChange={event=>onChange(event, setWeightKg)}/>
        </div>
        <div>
          <input type="text" value={height_cm} placeholder="Height (cm)" onChange={event=>onChange(event, setHeightCm)}/>
        </div>
        <div>
          <input type="text" value={age} placeholder="Age" onChange={event=>onChange(event, setAge)}/>
        </div>
