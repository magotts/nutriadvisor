...
const biometricsRouter = (db) => {
  ...
  router.put("/:id", async (req, res) => {
    try {
      ...
      const updateWeight = await db.query(
        `UPDATE biometrics SET weight = $1 WHERE id = $2 AND user_id = 1 RETURNING *`,
        [weight, id]
      );
  ...
  router.delete("/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const deleteBiometrics = await db.query(
        `DELETE from biometrics WHERE user_id = $1`,
        [id]
      );
  ...
  router.delete("/goal/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const deleteCoach = await db.query(`DELETE from goals WHERE id = $1`, [
        id,
      ]);
