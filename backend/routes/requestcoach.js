...
router.get("/coach/:id", async (req, res) => {
  ...
  try {
  const specificCoach = await db.query(`SELECT * from coaches WHERE goaltype_id = $1;`, [id]);
 ...
 router.post("/", async (req, res) => {
  try {
    ...
    const newCoach = await db.query(`INSERT INTO goals(user_id, coach_id, goaltype_id) VALUES ('1', $1, $2) RETURNING *`, [coach_id, goaltype_id]);
