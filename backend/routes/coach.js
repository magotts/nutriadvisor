...
const coachRouter = (db) => {
  ...
  router.get("/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const usersUnderCoach = await db.query(
        `select goals.id, users.*, coaches.*, goaltypes.* from users
  ...
  router.get("/userfooddiary/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const userFoodDiary = await db.query(
        `select * from food_diary where user_id = $1`,
        [id]
      );
