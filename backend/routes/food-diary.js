...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    const newFood = await db.query(`insert into food_diary(user_id, breakfast, lunch, dinner, snacks) VALUES (1, $1, $2, $3, $4) RETURNING *`, [breakfast, lunch, dinner, snacks]
    );
...
router.put("/:id", async (req, res) => {
  ...
  try {
  const {id} = req.params;
  const deleteFood = await db.query(`DELETE from food_diary WHERE id = $1`, [id]); 
